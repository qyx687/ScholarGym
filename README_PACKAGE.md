...
This branch is the fixed sequential scheduler variant. Graph events, deep
reranks, shadow Selector calls, and postprocess Ollama embedding calls execute
one at a time. All three scheduling parameters are locked to `1`; candidate
pools, formulas, Selector inputs, artifact schemas, and baseline feedback are
otherwise identical to the bounded-concurrency branch.
...
python code/eval.py \
  ...
  --postprocess_event_workers 1 \
  --postprocess_selector_concurrency 1 \
...
For an already-started run upgraded only to this packaged scheduler,
...
This branch fixes event workers, remote shadow Selector concurrency, and
postprocess Ollama embedding concurrency to `1`. Passing a larger value is
rejected by the CLI.
