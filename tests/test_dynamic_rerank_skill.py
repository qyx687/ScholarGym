...
def test_legacy_dashscope_credential_alias_is_host_guarded(monkeypatch):
    ...
    assert __import__("os").environ["DASHSCOPE_API_KEY"] == "test-secret"
    assert __import__("os").environ["DASHSCOPE_BASE_URL"].startswith(
        "https://dashscope.aliyuncs.com/"
    )


def test_legacy_provider_alias_rejects_non_dashscope_host(monkeypatch):
    monkeypatch.delenv("DASHSCOPE_API_KEY", raising=False)
    monkeypatch.delenv("DASHSCOPE_BASE_URL", raising=False)
    monkeypatch.setenv("DEEPSEEK_API_KEY", "test-secret")
    ...
    assert "DASHSCOPE_API_KEY" not in __import__("os").environ
    assert "DASHSCOPE_BASE_URL" not in __import__("os").environ
