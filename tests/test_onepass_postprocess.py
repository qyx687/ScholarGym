...
def test_sequential_graph_and_selector_scheduler_preserves_artifact_order(tmp_path):
    paper_db = {
        f"2001.0000{index}": {
            "title": f"seed {index}",
    ...
    with pytest.raises(ValueError, match="sequential package"):
        OnePassPostprocessor(
            selector=selector,
            paper_db=paper_db,
            writer=ArtifactWriter(str(tmp_path / "rejected"), "full"),
            s2_client=s2,
            per_subquery_processor=processor,
            deep_retrieval_processor=None,
            scoring_backend="bm25",
...
def test_dense_sequential_graph_keeps_postprocess_embedding_calls_serial(tmp_path):
...
def test_one_query_reuses_baseline_and_runs_graph_plus_both_deep_controls():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert deep_processor.max_active == 1
